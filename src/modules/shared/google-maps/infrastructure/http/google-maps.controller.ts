...
import {
  GeocodeQuerySchema, type GeocodeQuery,
  LatLngQuerySchema, type LatLngQuery,
  DirectionsQuerySchema, type DirectionsQuery,
  DistanceMatrixQuerySchema, type DistanceMatrixQuery,
  NearbyPlacesQuerySchema, type NearbyPlacesQuery,
  PlaceDetailsQuerySchema, type PlaceDetailsQuery,
  AutocompleteQuerySchema, type AutocompleteQuery,
  StaticMapQuerySchema, type StaticMapQuery,
...
export class GoogleMapsController {
  ...
  @Get('geocode')
  async geocode(@Query(new ZodValidationPipe(GeocodeQuerySchema)) query: GeocodeQuery) {
    return this.maps.geocodeAddress(query.address);
  }

  @ApiOperation({ summary: 'Reverse geocode coordinates to address' })
  @Get('reverse-geocode')
  async reverseGeocode(@Query(new ZodValidationPipe(LatLngQuerySchema)) query: LatLngQuery) {
  ...
  @Get('directions')
  async directions(@Query(new ZodValidationPipe(DirectionsQuerySchema)) query: DirectionsQuery) {
  ...
  @Get('distance-matrix')
  async distanceMatrix(@Query(new ZodValidationPipe(DistanceMatrixQuerySchema)) query: DistanceMatrixQuery) {
  ...
  @Get('places/nearby')
  async nearbyPlaces(@Query(new ZodValidationPipe(NearbyPlacesQuerySchema)) query: NearbyPlacesQuery) {
  ...
  @Get('places/details')
  async placeDetails(@Query(new ZodValidationPipe(PlaceDetailsQuerySchema)) query: PlaceDetailsQuery) {
  ...
  async addressAutocomplete(@Query(new ZodValidationPipe(AutocompleteQuerySchema)) query: AutocompleteQuery) {
  ...
  async queryAutocomplete(@Query(new ZodValidationPipe(AutocompleteQuerySchema)) query: AutocompleteQuery) {
    return this.maps.getQueryAutocomplete(query.input, query);
  }

  @ApiOperation({ summary: 'Get timezone info for coordinates' })
  @Get('timezone')
  async timezone(@Query(new ZodValidationPipe(LatLngQuerySchema)) query: LatLngQuery) {
  ...
  @Get('static-map')
  async staticMap(@Query(new ZodValidationPipe(StaticMapQuerySchema)) query: StaticMapQuery) {
