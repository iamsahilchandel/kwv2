...
import { S3_PORT, type IS3Port, type UploadResult, type DeleteResult } from './ports/s3.port.js';
...
export class S3Service {
  ...
  async uploadPublic(file: Express.Multer.File): Promise<UploadResult> {
    this.logger.log('Uploading public file to S3', { name: file.originalname, size: file.size, mime: file.mimetype });
    try {
      const result = await this.s3.uploadPublic(file);
      this.logger.log('Public file uploaded', { key: result.key });
      return result;
    } catch (err) {
      this.logger.error('S3 public upload failed', (err as Error).stack, { name: file.originalname });
      throw new ExternalServiceException('AWS S3', err);
    }
  }

  async uploadPrivate(file: Express.Multer.File): Promise<UploadResult> {
    this.logger.log('Uploading private file to S3', { name: file.originalname, size: file.size, mime: file.mimetype });
    try {
      const result = await this.s3.uploadPrivate(file);
      this.logger.log('Private file uploaded', { key: result.key });
      return result;
    } catch (err) {
      this.logger.error('S3 private upload failed', (err as Error).stack, { name: file.originalname });
      throw new ExternalServiceException('AWS S3', err);
    }
  }
  ...
  async deleteFiles(keys: string[]): Promise<DeleteResult> {
    ...
    try {
      ...
      this.logger.log('S3 delete complete', { deleted: result.deleted.length, failed: result.failed.length });
      return result;
    } catch (err) {
