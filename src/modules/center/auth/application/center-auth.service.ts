...
export class CenterAuthService {
  ...
  async login(user: IAuthUser, fcmToken: string) {
    this.logger.log('Center staff login', { staffId: user.id });
    ...
    this.logger.log('Center staff login successful', { staffId: user.id });
    return staff;
  }

  async logout(user: IAuthUser, fcmToken: string) {
    this.logger.log('Center staff logout', { staffId: user.id });
