...
export class LearnerEnrollmentsController {
  ...
  @Get('my')
  findAll(
    @Query(new ZodValidationPipe(QueryEnrollmentsSchema)) query: QueryEnrollmentsDto,
    @CurrentUser() user: IAuthUser,
  ) {
  ...
  @Patch(':id/cancel')
  cancel(@Param('id', ParseIntPipe) id: number, @CurrentUser() user: IAuthUser) {
    return this.service.cancel(user.id, id);
  }

  @Get(':id/payment-details')
  getPaymentDetails(@Param('id', ParseIntPipe) id: number, @CurrentUser() user: IAuthUser) {
