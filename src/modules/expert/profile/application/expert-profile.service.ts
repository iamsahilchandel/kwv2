...
export class ExpertProfileService {
  ...
  async getProfile(userId: number) {
    this.logger.log('Fetching expert profile', { expertId: userId });
  ...
  async register(firebaseUser: IFirebaseUser, body: RegisterExpertBody) {
    ...
    this.logger.log('Expert registered successfully', { expertId: expert.id });
  ...
  async updateProfile(user: IAuthUser, body: UpdateExpertProfileBody) {
    this.logger.log('Updating expert profile', { expertId: user.id });
    ...
    if (!expert.isVerified) {
      throw new ForbiddenException('Expert profile not yet verified by admin');
    }
    ...
    this.logger.log('Expert profile updated', { expertId: user.id });
    return { message: 'Profile updated successfully' };
  }

  async acceptTerms(userId: number) {
    this.logger.log('Expert accepting terms', { expertId: userId });
