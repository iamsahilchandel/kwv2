...
import type {
  CreateBatchClassMediaBody,
  QueryMediaQuery,
} from '../infrastructure/http/dto/expert-batch-class-media.dto.js';
...
export class ExpertBatchClassMediaService {
  ...
  async create(
    ...
    if (!batchClass)
      throw new NotFoundException(`Batch class with id ${classId} not found`);
  ...
  async remove(expertId: number, mediaId: number) {
    const media = await this.prisma.batchClassMedia.findUnique({
      where: { id: mediaId },
    });
